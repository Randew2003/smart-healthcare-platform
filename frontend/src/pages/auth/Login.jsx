        ...
        <div className="relative mx-auto grid min-h-screen max-w-7xl items-center gap-12 px-4 py-10 sm:px-6 lg:grid-cols-12 lg:px-8 lg:py-16">
          <section className="lg:col-span-5">
            ...
            <p className="mt-5 max-w-lg text-sm leading-7 text-slate-300 sm:text-base">
              Access your patient account, verified doctor workspace, or admin portal using the same secure backend login.
            </p>

            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {[
                "Shared backend login endpoint",
                "Role-aware redirect after login",
                "Secure AuthContext session storage",
                "Professional, footer-only layout"
              ].map((item) => (
                <div
                  key={item}
            ...
            <div className="rounded-3xl border border-white/10 bg-white/95 p-6 text-slate-900 shadow-2xl shadow-black/30 sm:p-8 lg:p-10">
              ...
              <div className="mt-6 grid gap-3 text-sm text-slate-600 sm:grid-cols-2">
                <div>
                  ...
                  <Link to="/register" className="font-semibold text-lime-700 transition hover:text-lime-800">
                    Get Started
                  </Link>
                </div>
                <div>
                  Register as a doctor?{" "}
                  <Link to="/register/doctor" className="font-semibold text-lime-700 transition hover:text-lime-800">
                    Apply here
                  </Link>
                </div>
              </div>
